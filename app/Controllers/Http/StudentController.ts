...
export default class StudentController {
  ...
  public async index({ request, response, auth }) {
    try {
      let user = await User.query()
        ...
        .first()
        .then((x) => x?.serialize())
      const users = await Student.listing(request, user)
  ...
  public async save({ request, response, auth }, record = null) {
    let loggedUser = await User.query()
      .where('id', auth.user.id)
      .preload('role')
      .first()
      .then((x) => x?.serialize())
    ...
    let country = await Country.query().where('id', data.country_id).preload('checklist').first()
    if (country) {
      if (!country.checklist.length) {
        return response.badRequest({ message: 'Application not processed of selected destination' })
      }
      data['total_check_list'] = country.checklist.filter(
        (x) => x.type === 'Gathering Checklist'
      ).length
    }
    ...
    if (data.lead_id) {
      await ContactRequest.query().where('id', data.lead_id).update({ is_student: 1 , status:'CONVERTED'})
    }
  ...
  public async show({ request, response }) {
    try {
      let student: any = await Student.query()
      ...
      let country = await Country.query()
        ...
        .first()
      if (country) {
        let checkList: any = country.checklist.map((x) => x.serialize())
      ...
      student['check_list']['Gathering Checklist'] = sortCheckList(
        student['check_list']['Gathering Checklist']
      )
   ...
   * @param response
   */
  public async studentInfo({ response, auth }) {
    try {
      let user = auth.user
      let student: any = await Student.query()
        .preload('selected_checklist')
   ...
   * @param response
   */
  public async updateStudentInfo({ request, response, auth }) {
    try {
      let user = auth.user
   ...
   * @param response
   */
  public async updateCheckList({ request, response, auth }) {
    try {
      const rules = {
      ...
      switch (type) {
        case 'Gathering Checklist':
          if (!steps_timestamp['check_list_started_on']) {
            steps_timestamp['check_list_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
        case 'Pre-CAS Process':
          if (!steps_timestamp['pre_cas_started_on']) {
            steps_timestamp['pre_cas_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }

          break
        case 'Financial Evidence':
          if (!steps_timestamp['financial_evidence_started_on']) {
            steps_timestamp['financial_evidence_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
        case 'Visa Application':
          if (!steps_timestamp['visa_app_started_on']) {
            steps_timestamp['visa_app_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
        case 'Pre Requisite':
          if (!steps_timestamp['pre_requisite_started_on']) {
            steps_timestamp['pre_requisite_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
        case 'After I-20':
          if (!steps_timestamp['after_i20_started_on']) {
            steps_timestamp['after_i20_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
        case 'Pre-Departure':
          if (!steps_timestamp['pre_departure_started_on']) {
            steps_timestamp['pre_departure_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
        case 'On-Arrival':
          if (!steps_timestamp['on_arrival_started_on']) {
            steps_timestamp['on_arrival_started_on'] = moment().format('DD/MM/YYYY h:mm:ss')
          }
          break
      ...
      let country = await Country.query().where('id', student.country_id).preload('checklist').first()
  ...
  public async updateCheckListValue({ request, response, auth }) {
    try {
      const rules = {
        student_id: 'required',
      ...
      let studentCheckList: any = await StudentChecklist.query()
      ...
      await await StudentChecklist.query()
        .where('student_id', payload.student_id)
        .where('check_list_id', payload.check_list_id)
        .update(data)
        .first()

      let checklist = await CheckList.findBy('id',payload.check_list_id)
  ...
  public async updateUniversity({ request, response,auth }) {
    try {
      const rules = {
  ...
  public async deleteUniversity({ request, response,auth }) {
    try {
      const rules = {
        university_id: 'required',
      }
  ...
  public async updateFinancialEvidence({ request, response,auth }) {
    try {
      const rules = {
        student_id: 'required',
  ...
  public async updateUniversityStatus({ request, response, auth }) {
    try {
      const rules = {
        value: 'url',
        status: 'string',
  ...
  async statistics({ request, response, auth }) {
    try {
      let user = await User.query()
        ...
        .first()
        .then((x) => x?.serialize())
      const stats = await Student.stats(request, user)
   ...
   * @param response
   */
  public async editView({ request, response }) {
    try {
      let student: any = await Student.query()
   ...
   * @param response
   */
  public async updateStudent({ request, response }) {
    try {
      let student: any = await Student.query().where('id', request.param('id')).first()
  ...
  public async updatecheckListStatus(id, auth, msg) {
    let student: any = await Student.query()
      .preload('selected_checklist')
      .preload('selected_universities', (query) => {
        query.preload('university', (query) => {
    ...
    for (let step of APPLICATION_PROCESS_LIST) {
  ...
  async destroy({ request, response }) {
    try {
      ...
      student['deleted_at'] = moment().utcOffset('+05:30').format('YYYY-MM-DD HH:mm:ss')
      await student.save()
      await User.query()
        .where('id', student.user_id)
        .update({
          active: false,
          deleted_at: moment().utcOffset('+05:30').format('YYYY-MM-DD HH:mm:ss'),
        })
    ...
    try {
      ...
      if (URLS.length) {
        ...
        for (let item of URLS) {
          ...
          await getFilesToLocal(
            item.url,
            folderPath + '/' + item.name.replace(/\s/g, '-').replace(/\//g, '-') + '.' + ext
          )
        }
        const outPath =
          Application.tmpPath('uploads') +
          ...
          student.id +
          '.zip'
        await zipDirectory(folderPath, outPath)
        let base64String = ''
        if (fs.statSync(outPath).isFile()) {
          base64String = await base64ToNode(
            fs.readFileSync(path.resolve(outPath)).toString('base64')
          )
        }
