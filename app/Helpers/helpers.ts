...
export function nextStage(country_id, stage) {
...
export function sendPasswordEmail(user, password) {
  let htmlContent = `
...
export function sortCheckList(checklist: any) {
...
export const getFilesToLocal = (url, path) => {
  return new Promise((resolve, _reject) => {
    try {
      https
        .request(url, function (response) {
          var data: any = new Transform()
          ...
          response.on('end', async function () {
            let err: any = await fs.writeFileSync(path, data.read())
            // console.log(err)
            if (!err) resolve(true)
          })
        })
...
export const APPLICATION_PROCESS_LIST = [
  {
