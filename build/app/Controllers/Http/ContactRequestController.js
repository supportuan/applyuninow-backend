...
class ContactRequestController {
    async prerequisite({ response }) {
        let countries = await Country_1.default.dropdown();
        let study_industry = await StudyIndustry_1.default.dropdown();
        let study_area = await StudyArea_1.default.dropdown();
        const data = {
        ...
        try {
            ...
            let lead = await ContactRequest_1.default.create({
                ...
                assigned_on: payload.assigned_to
                    ? (0, moment_1.default)().utcOffset('+05:30').format('YYYY-MM-DD HH:mm:ss')
                    : null,
            });
    ...
    async index({ request, response, auth }) {
        try {
            ...
            const companies = await ContactRequest_1.default.listing(request, user);
    ...
    async update({ request, response, auth }) {
        try {
            ...
            (contactrequest.email = payload.email),
                ...
                (contactrequest.study_area_id = payload.study_area_id || contactrequest.study_area_id),
    ...
    async show({ request, response }) {
        try {
            let contactrequest = await ContactRequest_1.default.query()
    ...
    async destroy({ request, response }) {
        try {
            const { contactrequest } = request;
            contactrequest['deleted_at'] = (0, moment_1.default)().utcOffset('+05:30').format('YYYY-MM-DD HH:mm:ss');
            await contactrequest.save();
    ...
    async statistics({ request, response, auth }) {
        try {
            let user = await User_1.default.query().where('id', auth.user.id).preload('role').first();
            const stats = await ContactRequest_1.default.stats(request, user);
    ...
    async updateStatus({ request, response, auth }) {
        try {
            const contactrequest = request.contactrequest;
            ...
            if (data.status === 'CALL_BACK') {
                ...
                contactrequest.callback_time = (0, moment_1.default)(data.callback_time)
                    .utcOffset('+05:30')
    ...
    async reassignLeadRequest({ request, response, auth }) {
        try {
            ...
            contactrequest.re_assign_date = (0, moment_1.default)().utcOffset('+05:30').format('YYYY-MM-DD HH:mm:ss');
            await contactrequest.save();
    ...
    async assignLead({ request, response, auth }) {
        try {
            ...
            contactrequest.assigned_on = (0, moment_1.default)().utcOffset(-330).format('YYYY-MM-DD HH:mm:ss');
...
exports.default = ContactRequestController;
